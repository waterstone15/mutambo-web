...
export default Controller.extend({
  ...
  title_string: computed('model.league.val.name', 'model.season.val.name', ->
    ...
    title = "#{l_name} → #{s_name} → Settings → Player Limit - Mutambo"
    return title
  )
  ...
  validateLimit: ->
    team_player_limit = get(@, 'model.season_settings.val.team_limits.team_players')
    limit = get(this, 'form.values.limit')
    ok = (
      (1 <= toNumber(limit) <= 100) &&
      (toNumber(limit) <= team_player_limit)
    )
  ...
  save: action(->
    ...
    reply = await @api.echo({
      data: obj
      path: "/v2/season-settings/roster-player-limit/update"
    })
