    ...
    .flex.flex-col.w-full
      .px-8.flex.justify-between.items-center.border-b.border-neutral-100 class='min-h-[80px]'
        .inline-flex.items-center.flex-wrap class='md:h-[48px] py-4 md:py-0'
          ...
          % Breadcrumb [
            @goto={hash route='app.league.season.settings.edit-roster-player-limit' query=(hash league_id=model.league.meta.id season_id=model.season.meta.id)}
            @text='Player Limit'
          ]
            ...
            = if form.help.limit
              .bg-neutral-100.rounded-md.mt-1.mb-2.pl-3.py-3.pr-4
                ol.list-disc.ml-5.space-y-2
                  li.text-neutral-600: | Player limit is the maximum number of players that can be on a game sheet.
            ...
            = if (and (not form.valid.limit) form.submitted)
              .bg-red-100.rounded-md.mt-1.mb-2.pl-3.py-3.pr-4
                  ol.list-disc.ml-5.space-y-2
                    ...
                    li.text-red-800 The limit must be less than or equal to the team player limit ({{get this 'model.season_settings.val.team_limits.team_players'}}).
