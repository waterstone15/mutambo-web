    ...
    .flex.flex-col.w-full
      .px-8.flex.justify-between.items-center.border-b.border-neutral-100 class='min-h-[80px]'
        .inline-flex.items-center.flex-wrap class='md:h-[48px] py-4 md:py-0'
          ...
          % Breadcrumb [
            @goto={hash route='app.league.season.settings.edit-team-manager-limit' query=(hash league_id=model.league.meta.id season_id=model.season.meta.id)}
            @text='Team Manager Limit'
          ]
            ...
            = if form.help.limit
              .bg-neutral-100.rounded-md.mt-1.mb-2.pl-3.py-3.pr-4
                ol.list-disc.ml-5.space-y-2
                  li.text-neutral-600 Team manager limit is the maximum number of managers that can be on a team.
                  li.text-neutral-600 The limit must be a number from 1 to 10.

            = if (and (not form.valid.limit) form.submitted)
              .bg-red-100.rounded-md.mt-1.mb-2.pl-3.py-3.pr-4
                  ol.list-disc.ml-5.space-y-2
                    li.text-red-800 The limit must be a number from 1 to 10.

            .mt-1.relative.rounded-md
              % Input{on 'input' (fn this.formValueChanged 'limit')}{on 'keydown' (fn this.formKeyPress 'limit')} @type='number' @value=form.values.limit name='team-manager-limit' class='form-input bg-transparent block border-neutral-300 mutambo-focus-sky focus:z-10 relative rounded-tr-md rounded-md w-full' placeholder='' min=1 max=10
