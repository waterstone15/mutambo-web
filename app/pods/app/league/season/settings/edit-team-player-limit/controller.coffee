...
export default Controller.extend({
  ...
  title_string: computed('model.league.val.name', 'model.season.val.name', ->
    ...
    title = "#{l_name} → #{s_name} → Settings → Team Player Limit - Mutambo"
    return title
  )
  ...
  validateLimit: ->
    roster_player_limit = get(@, 'model.season_settings.val.team_limits.rostered_players')
    limit = get(this, 'form.values.limit')
    ok = (
      (1 <= toNumber(limit) <= 100) &&
      (toNumber(limit) >= roster_player_limit)
    )
  ...
  save: action(->
    ...
    reply = await @api.echo({
      data: obj
      path: "/v2/season-settings/team-player-limit/update"
    })
