    ...
    .flex.flex-col.w-full
      .px-8.flex.justify-between.items-center.border-b.border-neutral-100 class='min-h-[80px]'
        .inline-flex.items-center.flex-wrap class='md:h-[48px] py-4 md:py-0'
          ...
          % Breadcrumb [
            @goto={hash route='app.league.season.misconduct.edit' query=(hash league_id=model.league.meta.id season_id=model.season.meta.id misconduct_id=model.misconduct.meta.id)}
            @text='Edit Misconduct'
          ]
        ...
        .space-y-6 class='sm:w-full sm:max-w-md'
          div
            label.flex.items-center.justify-between for='status'
              .font-medium.text-neutral-900: | Status
              / button{on 'click' (fn this.toggleHelp 'status')} class='ml-2 p-2 rounded-full flex items-center justify-center text-neutral-500 hover:bg-neutral-200 hover:text-neutral-700 focus:outline-none focus:ring-2 focus:ring-sky-500'
              ...
              .mt-2
                div
                  label.inline-flex.items-center class='cursor-pointer mb-2'
                    input{on 'input' (fn this.formValueChanged 'status' 'suspended')} checked={eq this.form.values.status 'suspended'} type='radio' name='radio-direct' class='form-radio border-neutral-300 text-sky-600 shadow-sm focus:border-sky-300 focus:ring focus:ring-offset-0 focus:ring-sky-200 focus:ring-opacity-50' value=this.form.values.status
                    span.ml-2: | Suspended
                  div
                  label.inline-flex.items-center class='cursor-pointer mb-2'
                    input{on 'input' (fn this.formValueChanged 'status' 'resolved')} checked={eq this.form.values.status 'resolved'} type='radio' name='radio-direct' class='form-radio border-neutral-300 text-sky-600 shadow-sm focus:border-sky-300 focus:ring focus:ring-offset-0 focus:ring-sky-200 focus:ring-opacity-50' value=this.form.values.status
                    span.ml-2: | Resolved
                  div
