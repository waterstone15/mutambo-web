= page-title 'Manager Invite - Mutambo' replace=true separator=' → '
    ...
    .flex.flex-col
      .flex.flex-col.justify-center.py-8 class='sm:px-6 lg:px-8'
        ...
        div class='sm:mx-auto sm:w-full sm:max-w-md'
          h2.mt-6.mx-6.text-center.text-2xl.font-extrabold.text-neutral-900: | Team Manager
          h2.mt-2.mx-6.text-center.text-xl.font-medium.text-neutral-900
            = if model.itm.val.team.val.name
              | {{model.itm.val.team.val.name}}
            = if model.itm.val.league.val.name
              | {{' • '}}{{model.itm.val.league.val.name}}
            = if model.itm.val.season.val.name
              | {{', '}}{{model.itm.val.season.val.name}}
        ...
        .mt-8.mb-32 class='sm:mx-auto sm:w-full sm:max-w-md'
          .bg-white.border.border-neutral-200.px-6.py-10.mx-2.rounded-lg class='sm:rounded-lg sm:px-10'
            = if model.itm
              p
                | You are invited to be a team manager of
                | {{' '}}
                span.font-semibold {{model.itm.val.team.val.name}}
                = if model.itm.val.league.val.name
                  | {{' in the '}}
                  span.font-semibold {{model.itm.val.league.val.name}}
                = if model.itm.val.season.val.name
                  | {{', '}}
                  span.font-semibold {{model.itm.val.season.val.name}}
                  | {{' '}}season.
            = else
            ...
            .flex.flex-row.mt-6.space-x-2
              ...
              a href='/register/team-manager{{model.hash}}' class='w-full flex justify-center py-2 px-2 border border-transparent rounded-md shadow-sm font-medium text-white bg-sky-600 hover:bg-sky-700 mutambo-focus-sky-dark' role='link' tabindex='0'
                span: | Continue
