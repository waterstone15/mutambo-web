= page-title 'Free Agent Invite - Mutambo' replace=true separator=' → '
      ...
      .flex.flex-col.justify-center.py-8 class='sm:px-6 lg:px-8'
        ...
        .flex.justify-center
          .w-32.h-32.rounded-full.flex.items-center.justify-center.bg-white.border.border-neutral-200
            % FaIcon class='text-purple-400' @fixedWidth=true @icon='running' @prefix='fas' @size='3x' @transform='grow-0'
        ...
        .mt-8.mb-32 class='sm:mx-auto sm:w-full sm:max-w-md'
          .bg-white.border.border-neutral-200.px-6.py-10.mx-2.rounded-lg class='sm:rounded-lg sm:px-10'
            ...
            = if model.league
              p
                span You are invited to be a free agent in the{{' '}}
                span.font-semibold {{model.league.val.name}}
                span .
            = else
              p This invite link is not valid. Please contact a league administrator and ask them for a new link.
            ...
            .flex.flex-row.mt-6.space-x-2
              button{on 'click' (fn this.ignore)} class='w-full flex justify-center py-2 px-2 border border-neutral-200 rounded-md shadow-sm font-medium text-neutral-900 bg-white hover:border-neutral-300 hover:bg-neutral-50 mutambo-focus-sky' role='button' tabindex='0'
                span Ignore
              a href='/register/league-free-agent{{model.hash}}' class='w-full flex justify-center py-2 px-2 border border-transparent rounded-md shadow-sm font-medium text-white bg-sky-600 hover:bg-sky-700 mutambo-focus-sky-dark' role='link' tabindex='0'
                span Continue
