...
export default Service.extend({
  ...
  _load: ({ end_before, search_at, season_id, start_after }) ->
    ...
    reply = await this.api.echo({ path: '/v1/-/schedule/retrieve', data: obj, auth: false })

    { start, end, first, last, games, league, season } = reply
    return { start, end, first, last, games, league, season }
